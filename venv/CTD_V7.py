...
def plot(data):
    ...
    plt.tight_layout()
    plt.savefig("Perfil_bruto.png", format='png', dpi=900, transparent=False)
...
class DataProcessor:
    ...
    def downcast(self):
        ...
        idx_max_pressure = self.data['pressure'].idxmax()

        # Atualiza o DataFrame apenas com os dados de downcast
        downcast_data = self.data.loc[:idx_max_pressure]

        print('A maior pressão encontrada foi: ' + str(self.data['pressure'].iloc[idx_max_pressure]) + ' dBar')
...
def bin_average(data, tamanho_janela, coluna, coluna2):
    ...
    for i in range(0, len(data), tamanho_janela):
        ...
        if i + tamanho_janela - 1 < len(data):
            ...
            media_primeiro_ultimo = np.mean([data['pressure'].iloc[i], data['pressure'].iloc[i + tamanho_janela - 1]])
...
def plot_binned(data):
    ...
    plt.tight_layout()
    plt.savefig("Perfil_binado.png", format='png', dpi=900, transparent=False)
...
'''
data = pd.read_csv('/home/labdino/PycharmProjects/CTDprocessing/venv/Dados/FILE17V2.csv', delimiter='\t', index_col=False)
processor = DataProcessor(data)
data_processada = processor.process_data(10.12)
data_processada.to_csv('/home/labdino/PycharmProjects/CTDprocessing/venv/dados_pós/resultado_preprocessado.csv')
'''=================================================================================================================='''
'''
...
'''
start_time = time.perf_counter()
binado = bin_average(data_processada, 5, 'temperature', 'salinity')
end_time = time.perf_counter()
elapsed_time = end_time - start_time
...
binado.to_csv('/home/labdino/PycharmProjects/CTDprocessing/venv/dados_pós/resultado_binado.csv')
'''=================================================================================================================='''
plot(data)

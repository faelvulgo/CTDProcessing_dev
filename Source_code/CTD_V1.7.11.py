"""
A Python package for CTD data pre-processing.
"""
import pandas as pd
import numpy as np
...
def plot(data, path):
    """
    Plot a simple temperature profile.
...
def convert(data):
    """
    For each dataframe column, except the 'time' column, substitute ',' for '.', and take out the '.' used as thousands separator.

    Args:
        data (pandas dataframe): a pandas dataframe with the CTD data
...
def downcast(data):
    """
    Update the dataframe maintaining only the downcast data.

    Args:
        data (pandas dataframe): a pandas dataframe with the CTD data

    Returns: the new dataframe with only downcast data
    """
...
def remove_outliers(data):
    """
    Remove the data outliers using the 3-sigma method

    Args:
        data (pandas dataframe): a pandas dataframe with the CTD data

    Returns: an updated dataframe without the outliers
    """
    mean = data.mean()
    std = data.std()
...
def above_sea_level(data, sea_level_pressure):
    """
    Remove the data collected above the sea level.

    Args:
        data (pandas dataframe): a pandas dataframe with the CTD data
        sea_level_pressure (float): the sea level pressure in units of dBar

    Returns: an updated dataframe without the values above the sea_level_pressure
    """
...
def pressure_loops(data):
    """
    Remove the pressure loops.

    Args:
        data (pandas dataframe): a pandas dataframe with the CTD data

    Returns: an updated dataframe without the pressure loops
    """
...
def bin_average(data, tamanho_janela, coluna, coluna2, lat):
    ...
    Args:
        data (pandas dataframe): a pandas dataframe with the CTD data
        tamanho_janela (int): the size of the bin in depth meters
        coluna: the first column to be binned
        coluna2: the second column to be binned
        lat (int): latitude

    Returns: an updated dataframe with the binned data
    """
...
def plot_perfil_termosal(data, path):
    """
    Plot a temperature and salinity profile together.

    Args:
        data (pandas dataframe): a pandas dataframe with the CTD data
        path (directory and file name): where and how will the plot be saved on the computer
    """
...
def diagramats(data, path, lon, lat):
    """
    Plot a T-S diagram with density isolines.
